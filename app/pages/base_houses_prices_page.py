...
class BaseHousesPricesPage(BasePage):
    ...
    def _render_data_graph(cls, class_name):
        return html.Div(
                    [
                    dcc.Graph(
                        id=cls.KEYS.GRAPH, figure=cls._get_houses_price_graph(), className='chart-chart'
                        ),
                    html.Div([html.A('', target='_blank', id=cls.KEYS.OFFER_LINK, href=''),], className="offer-textholder"),
                    ], className=class_name
                )
    ...
    def _render_area_range_slider(cls, class_name):
        ...
        return html.Div(
                    ...
                    dcc.RangeSlider(
                        id = cls.KEYS.AREA_SLIDER, marks = areas, min=min_area, max=max_area, value=[0, int(len(areas)/2)], className = 'slider', step=None,
                        ),
                    ],className=class_name
                )
    ...
    def _render_price_from_dropdown(cls, class_name):
        ...
        return html.Div(
                    dcc.Dropdown(id=cls.KEYS.PRICE_FROM, options=prices_from, value=prices_from[0].get("value"), className=class_name),
                )
    ...
    def _render_price_to_dropdown(cls, class_name):
        ...
        return html.Div(
                    dcc.Dropdown(id=cls.KEYS.PRICE_TO, options=prices_to, value=prices_to[-1].get("value"), className=class_name),         
                )
