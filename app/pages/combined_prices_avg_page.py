...
from data_loaders import AverageHousesPricesLoader, CityHousesLoader
...
class CombinedPricesAveragesPage(BaseHousesPricesPage):
    ...
    def _render_avg_radios(cls, class_name):
        ...
        return dcc.RadioItems(  id = cls.KEYS.GROUP_AVG_RADIO,
    ...
    def _get_houses_price_graph(cls):
        ...
        fig.for_each_trace(
            lambda trace:
                ...
                if (trace.name in full_price and trace.name=="Full price") else full_price.add(trace.name))
        fig.update_layout(
    ...
    def _make_scatter(cls, data):
        ...
        for label, market in data.items():
            traces.append(go.Scatter(
                ...
                xaxis='x2', 
                legendgroup="avg_price_mk", legendgrouptitle_text="Average price per m²"
                ))
